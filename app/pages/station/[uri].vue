<template>
    ...
    <div class="select-none mx-auto w-full h-full flex flex-col items-center pt-4">
        <NuxtLink @vue:unmounted="stopRadio" class="link link-info" to="/">
            Home
        </NuxtLink>


        <h1 class="my-5">{{ radioName }}</h1>
...
function actionBtn() {
    ...
    if (isPlaying.value == true) {
        ...
        radio.src = `https://station.nightcore.ir/${route.params.uri}`
...
async function getData() {
    ...
    serverData.source.forEach(element => {

        if (element.server_url == `https://station.nightcore.ir/${route.params.uri}`) {

            radioName.value = element.server_name;
            musicName.value = element.title;
            listenersCount.value = element.listeners
        }
    });
    useSeoMeta({ title: `${radioName.value} | ${musicName.value}` })
}
</script>